...
function getSmtpConfig(): { host: string; port: number; user: string; pass: string } {
    const fromEnv = process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS;
    const host = fromEnv ? process.env.SMTP_HOST! : (functions.config().smtp as { host?: string })?.host;
    const port = fromEnv ? (process.env.SMTP_PORT || '587') : (functions.config().smtp as { port?: string })?.port || '587';
    const user = fromEnv ? process.env.SMTP_USER! : (functions.config().smtp as { user?: string })?.user;
    const pass = fromEnv ? process.env.SMTP_PASS! : (functions.config().smtp as { pass?: string })?.pass;
    if (!host || !user || !pass) {
        throw new functions.https.HttpsError(
            'failed-precondition',
            'SMTP is not configured. Set SMTP_HOST, SMTP_USER, SMTP_PASS in .env (or Firebase config smtp.host, smtp.user, smtp.pass).'
        );
    }
    return {
        host,
...
async function doRequestPasswordResetOtp(email: string): Promise<{ success: true }> {
...
async function doVerifyOtpAndResetPassword(email: string, otp: string, newPassword: string): Promise<{ success: true }> {
    const docId = email.replace(/[.@]/g, '_');
    ...
    const expiresAt = d.expiresAt as admin.firestore.Timestamp;
 ...
 */
export const requestPasswordResetOtp = functions.https.onCall(async (data: { email?: string }) => {
 ...
 */
export const verifyOtpAndResetPassword = functions.https.onCall(async (data: { email?: string; otp?: string; newPassword?: string }) => {
    const email = typeof data?.email === 'string' ? data.email.trim().toLowerCase() : '';
...
export const refreshCustomClaims = functions.https.onCall(async (_, context) => {
    ...
    const roles: string[] = doc.data()?.roles ?? [];
    await admin.auth().setCustomUserClaims(uid, { roles });
    return { updated: true, roles };
});
...
function getGeminiKey(): string {
    const key = process.env.GEMINI_API_KEY || (functions.config().gemini as { api_key?: string })?.api_key;
 ...
 */
export const processReportImage = functions.https.onCall(async (data: { imageBase64?: string }, context) => {
    ...
    try {
        const request = {
            ...
            features: [
                ...
                { type: 'LANDMARK_DETECTION' as const, maxResults: 1 }
            ]
        };
 ...
 */
export const suggestWasteType = functions.https.onCall(async (data: { imageDataUrl?: string }, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Must be signed in');
    ...
    try {
        ...
        if (!imageDataUrl) return { suggestedType: 'others' };
 ...
 */
export const processVoiceReport = functions.https.onCall(async (data: { transcript?: string; lang?: string }, context) => {
    if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Must be signed in');
...
export const onReportUpdated = functions.firestore
    ...
    .onUpdate(async (change, context) => {
        ...
        if (!afterPhotoJustSet || !hasBeforePhoto) return;
        ...
        try {
            ...
            const fetchBase64 = async (url: string): Promise<string> => {
                ...
                if (!pathMatch) throw new Error(`Cannot parse Storage URL: ${url}`);
...
- If image quality too poor to judge, set confidence < 0.4`;
            ...
            let result: { is_cleaned?: boolean; confidence?: number; improvement?: string; note?: string } = {};
            try {
...
const AREA_BASE_SCORES: Record<string, number> = {
    TEMPLE_ZONE: 60,
    MARKET: 55,
    BUS_STAND: 55,
    TASMAC_ADJACENT: 50,
...
function getSuggestedWorkers(risk: string): number {
...
export const dailyWastePrediction = functions.pubsub
    ...
    .onRun(async () => {
        ...
        const festivalZoneIds = new Set<string>();
        ...
        streetsSnap.docs.forEach(streetDoc => {
            ...
            const areaType: string = street.areaType ?? 'RESIDENTIAL_LOW';
            let score = AREA_BASE_SCORES[areaType] ?? 20;
            const reasons: string[] = [areaType];

            if (isWeekend) { score += 20; reasons.push('WEEKEND'); }
            if (isMonsoon) { score += 10; reasons.push('MONSOON_SEASON'); }
            ...
            if (hist > 2) reasons.push('HISTORICALLY_HIGH');
